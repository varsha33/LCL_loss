...
def train(epoch, train_loader, model_main, model_helper, loss_function, optimizer, lr_scheduler, log):
    ...
    for idx, batch in enumerate(train_loader):
        ...
        if steps % 100 == 0:
            print(
                f'Epoch: {epoch:02}, Idx: {idx + 1}, Training Loss_1: {loss_1.item():.4f},Training Loss_2: {loss_2.item():.4f}, Time taken: {((time.time() - start_train_time) / 60): .2f} min')
            start_train_time = time.time()
    ...
    return train_loss_1 / len(train_loader), train_loss_2 / len(train_loader), total_epoch_acc_1 / len(
        train_loader), total_epoch_acc_2 / len(train_loader), acc_curve_1, acc_curve_2
...
def test(epoch, test_loader, model_main, model_helper, loss_function, log):
    ...
    f1_score_emo_2 = 100 * f1_score(total_emo_true, total_emo_pred_2, average="macro")

    f1_score_emo_1_w = 100 * f1_score(total_emo_true, total_emo_pred_1, average="weighted")
    f1_score_emo_2_w = 100 * f1_score(total_emo_true, total_emo_pred_2, average="weighted")
    ...
    return total_epoch_acc_1 / len(test_loader), total_epoch_acc_2 / len(
        test_loader), f1_score_1, f1_score_2, save_pred, acc_curve_1, acc_curve_2


def lcl_train(log, save_home):
    ...
    model_helper = weighting_network(log.param.batch_size, log.param.hidden_size, log.param.emotion_size,
                                     log.param.model_type)
    ...
    for epoch in range(1, log.param.nepoch + 1):
        ...
        train_loss_1, train_loss_2, train_acc_1, train_acc_2, train_acc_curve_1, train_acc_curve_2 = train(epoch,
                                                                                                           train_data,
                                                                                                           model_main,
                                                                                                           model_helper,
                                                                                                           losses,
        ...
        val_acc_1, val_acc_2, val_f1_1, val_f1_2, val_save_pred, val_acc_curve_1, val_acc_curve_2 = test(epoch,
                                                                                                         valid_data,
                                                                                                         model_main,
                                                                                                         model_helper,
                                                                                                         losses, log)

        test_acc_1, test_acc_2, test_f1_1, test_f1_2, test_save_pred, test_acc_curve_1, test_acc_curve_2 = test(epoch,
                                                                                                                test_data,
                                                                                                                model_main,
                                                                                                                model_helper,
                                                                                                                losses,
